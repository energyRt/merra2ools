...
eq_time <- function(yday, check_yday = TRUE) {
  # E.qt - the equation of time (???)
  # E.qt <-
  # (n<107) * (-14.2*sinpi((n+7)/111)) +
  # (n>246) * 16.4*sinpi((n-247)/113) +
  # (n>106)*(n<167) * 4.0*sinpi((n-106)/59) +
  # (n>166)*(n<247) * (-6.5*sinpi((n-166)/80))
...
#' @examples
#' NA
solar_position <- function(x = NULL,
                           datetime = NULL, # tz = "UTC",
                           yday = NULL, hour = NULL, 
                           lon = NULL, lat = NULL,
                           ALL = TRUE, verbose = TRUE) {
  # browser()
  if (!is.null(x)) {
    stopifnot(is.null(datetime))
  ...
  if (is.null(x$datetime)) {
    ...
    if (any(lubridate::tz(x$datetime) != "UTC")) {
      if (verbose) cat("   changing the timezone of 'datetime' to UTC\n")
      x$datetime <- lubridate::with_tz(x$datetime, tzone = "UTC")
    }
    if (verbose) cat("   Day of the year (yday) & Hour (hour) at UTC timezone\n")
  ...
  solar_time0 <- x$hour + eq_time / 60 + x$lon / 15
  solar_time1 <- x$hour + 1 + eq_time / 60 + x$lon / 15
  # Hour angle ####
  # x$hour_angle <- hour_angle(x$solar_time)
  if (verbose) cat("   Hour angle (hour_angle)\n")
  hour_angle <- pi * (solar_time - 12) / 12
  hour_angle0 <- pi * (solar_time0 - 12) / 12
  hour_angle1 <- pi * (solar_time1 - 12) / 12
  # Zenith angle ####
  ...
  zenith <- acos(sinpi(x$lat / 180) * sin(declination) +
                   cospi(x$lat / 180) * cos(declination) * cos(hour_angle)) / pi * 180
  # zenith0 <- acos(sinpi(x$lat / 180) * sin(declination) +
  #                   cospi(x$lat / 180) * cos(declination) * cos(hour_angle0)) / pi * 180
  # zenith1 <- acos(sinpi(x$lat / 180) * sin(declination) + 
  #                   cospi(x$lat / 180) * cos(declination) * cos(hour_angle1)) / pi * 180
  
  # zenith_daytime <- (zenith0 < 90) & (zenith1 < 90)
  
  # zenith_avr <- zenith1
  # ii <- zenith0 < zenith1
  # zenith_avr[ii] <- zenith0[ii]
  # Azimuth angle ####
  ...
  # x$yday <- yday
  # x$hour <- hour
  # x$zenith_avr <- zenith_avr
  # x$daytime <- zenith_daytime

...
wpc <- tibble( 
  mps = 0:30, # wind speed m/s
  af = fWPC(mps) # availability factor [0,1]
)

fig <- ggplot(wpc) + 
  geom_line(aes(x = mps, y = af), colour = "red", size = 2) +
...
wind_plot(dat2, "af50", 1, limits = c(0, 1), 
          palette = "PuBu", direction = 1, legend.name = "AF")
wind_plot(dat2, "af10", 1, limits = c(0, 1),
          palette = "BuPu", direction = 1, legend.name = "AF")
wind_plot(dat2, "af100", 1, limits = c(0, 1),
          palette = "Blues", direction = 1, legend.name = "AF")
wind_plot(dat2, "af200", 1, limits = c(0, 2))
...
```{r, fig.cap="Wind speed at 50 meters height, m/s", echo=FALSE}
gif2 <- "../man/merra_wind_af50_12fps.gif"
